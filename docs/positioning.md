...
v0.1.5 的基准修订把这个"全包"钉成能力矩阵,不是一个笼统的 `transport ready`:每个 harness 必须声明 `headless`、`acp`、`direct_user_session` 三类运行面的支持状态和 isolation readiness。Happy/Happier 这类 product session,以及 Warp/cmux/tmux 这类 terminal session,都属于 `direct_user_session`;它们必须显式声明 attach/inject/interrupt/cancel 能力和 session/route 隔离边界,不能因为能看到终端输出就默认可控。
