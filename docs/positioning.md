...
**HOLP（Human On Loop Protocol）= 开源、免费、本地优先的 multi-agent 编排协议,加一个参考 daemon 和参考 consumer。**
...
v0.1.5 的基准修订把这个"全包"钉成能力矩阵,不是一个笼统的 `transport ready`:每个 harness 必须声明 `headless`、`acp`、`direct_user_session` 三类运行面的支持状态和 isolation readiness。Happy/Happier 这类 product session,以及 Warp/cmux/tmux 这类 terminal session,都属于 `direct_user_session`;它们必须分开声明 attach/observe/read 观察能力与 inject/interrupt/cancel 控制能力,并给出 session/route 隔离边界,不能因为能看到终端输出就默认可控。
...
参考实现 **v0.1.x 只做 Local**。Remote 是 Blueprint M12 的 planned surface,未来对位 [spawn](https://github.com/OpenRouterLabs/spawn) 的 agent×cloud 矩阵、[skypilot](https://github.com/skypilot-org/skypilot) 的跨云算力调度——不进 v0.1.x wire。
...
| `flock.declare`/`flock.discover` | 综合抽象自 **Oz** `Harness` oneof / **happier** catalog / **spawn** manifest / **cmux** 进程扫描 | **非直接对位**:四者分别给 harness enum / backend catalog / agent-cloud manifest / 硬编码 provider,HOLP 的「声明+发现+实测」协议面是综合抽象,不是任何一个的搬运。v0.1.5 进一步要求 flock 返回 runtime surface / isolation readiness matrix,避免只用 transport/status 调度 |
...
- **v0.1.x 不做 Remote**:Remote 不进 v0.1.x wire(不留半截 opaque 占位),实现+协议归入 Blueprint M12 的独立扩展。
- **不绑定任何终端/APP**:协议 vendor-neutral,谁都能接(cmux/Warp/happier/CLI)。
- **不把 direct session 当隐式能力**:Happy/Happier 产品会话和 Warp/cmux/tmux 终端会话都可以成为 HOLP consumer/session 接入面,但必须显式声明 direct channel 能力和隔离边界。
