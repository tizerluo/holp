...
- **why bundle**:PR9 spec 明示 *"`mcp-codex` reviewer 只有在可用 read-only enforced session 时才可执行"*,而 #17 已把 Codex app-server 从一次性脆弱模型推进到基础 turn recovery,并为后续 read-only sandbox 启动路径留出 adapter option。下一步 PR9 仍必须补 enforcement attestation + reviewer parser,不能把 default `workspace-write` 当 read-only reviewer。
- **顺序**:#17 已作为健壮性底盘落地 → PR9(在底盘上接 reviewer)
...
注:#17 基础 runtime recovery 已落地;PR9 仍要在此基础上补 read-only enforcement attestation 和 canonical reviewer parser。loopwright V2.4 的 failure taxonomy / availability feedback(`fb8a0dd` / `bea0f5c`)已作为 transient / unavailable 分类参考,但没有搬它的整套 orchestrator。
...
**关键路径**:PR9 → PR11 → PR12,同时 PR10 可并行提前落地;#17 已作为独立底盘 PR 落地。

**P0 阻塞依赖**:#17 已解除 PR9 的 stdio/turn recovery 底盘阻塞;PR9 仍受 read-only enforcement attestation 和 reviewer parser 阻塞。
...
**当前最该启动的是 PR10 或 PR9**:PR10 可先补 fake/M5 consumer CLI 体验;PR9 可在 #17 底盘上接真实 reviewer pilot。PR11 必须在 PR9 之后才能跑通真实 cross-vendor consensus,PR12 的完整 matrix 展示应在 PR10+PR11 后做。其余全是参考素材或远期工作。
