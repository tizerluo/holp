...
| Cursor Agent | `degraded` until #51 records matching headless smoke evidence. | `degraded` until #51 records real ACP terminal smoke evidence. | `degraded` by default; #50 wires tmux direct and upgrades only after `HOLP_REAL_HARNESS_DIRECT_SMOKE=1` proves HOLP-owned session + `HOLP_OK`. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Kimi Code | `degraded` until #51 records matching headless smoke evidence. | `degraded` until #51 records real ACP terminal smoke evidence. | `degraded` by default; #50 keeps tmux direct and upgrades only after `HOLP_REAL_HARNESS_DIRECT_SMOKE=1` proves HOLP-owned session + `HOLP_OK`. | `adapters/first-batch-harnesses.ts` Kimi `direct` definition; `adapters/direct-tmux.ts`; `scripts/smoke/first-batch-direct.ts`. |
| OpenCode | `degraded` until #51 records matching headless smoke evidence. | `degraded` until #51 records real ACP terminal smoke evidence. | `degraded` by default; #50 wires tmux direct with an explicit model and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Pi | `degraded` until #51 records matching headless smoke evidence. | `degraded` until #51 records real `pi-acp` terminal smoke evidence. | `degraded` by default; #50 wires tmux direct with pinned provider/model args and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Reasonix | `degraded` until #51 records matching headless smoke evidence. | `degraded`; `session/new` / prompt terminal path is not stable enough to count ready. | `degraded` by default; #50 wires tmux direct with pinned model args and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts` Reasonix degraded reason path; `scripts/smoke/first-batch-direct.ts`. |
