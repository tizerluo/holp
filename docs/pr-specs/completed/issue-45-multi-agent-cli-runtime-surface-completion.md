...
| Claude Code | `degraded` by default; `ready` after headless probe succeeds (`claude --version` + capability probe + write-denial enforcement). #49 wired. | `unsupported`; Claude Code has no native ACP. Explicit acp selection returns `undefined` from registry, never falls back to headless. #49 wired. | `degraded` by default (`claude_direct_smoke_not_enabled`); upgrades only after `HOLP_REAL_CLAUDE_SMOKE=1` holp-owned tmux probe succeeds. `coder_worktree` requires smoke opt-in; `read_only_review` additionally requires `direct_reviewer_execution_config`. #49 wired. | `adapters/claude-code.ts` claudeRuntimeSurfaces(); `adapters/registry.ts` native-claude per-surface map; `scripts/smoke/claude-surfaces.ts`. |
| Cursor Agent | `degraded` until #51 records matching headless smoke evidence. | `degraded` by default (`acp_smoke_not_enabled`); #51 requires real `cursor-agent acp` JSON-RPC terminal evidence and records auth-required reasons without using headless output. | `degraded` by default; #50 wires tmux direct and upgrades only after `HOLP_REAL_HARNESS_DIRECT_SMOKE=1` proves HOLP-owned session + `HOLP_OK`. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Kimi Code | `degraded` until #51 records matching headless smoke evidence. | `degraded` by default (`acp_smoke_not_enabled`); #51 wires `kimi acp` with `cwd + mcpServers: []` and upgrades only after JSON-RPC terminal `HOLP_OK`. | `degraded` by default; #50 keeps tmux direct and upgrades only after `HOLP_REAL_HARNESS_DIRECT_SMOKE=1` proves HOLP-owned session + `HOLP_OK`. | `adapters/first-batch-harnesses.ts` Kimi `direct` definition; `adapters/direct-tmux.ts`; `scripts/smoke/harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| OpenCode | `degraded` until #51 records matching headless smoke evidence. | `degraded` by default (`acp_smoke_not_enabled`); #51 wires `opencode acp --pure` with `cwd + mcpServers: []` and upgrades only after JSON-RPC terminal `HOLP_OK`. | `degraded` by default; #50 wires tmux direct with an explicit model and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Pi | `degraded` until #51 records matching headless smoke evidence. | `degraded` by default (`acp_smoke_not_enabled`); #51 wires `pi-acp` and upgrades only after real ACP JSON-RPC terminal `HOLP_OK`. | `degraded` by default; #50 wires tmux direct with pinned provider/model args and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts`; `scripts/smoke/harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
| Reasonix | `degraded` until #51 records matching headless smoke evidence. | `degraded`; #51 sends `reasonix acp --model deepseek-flash` with `session/new` cwd-only params and still does not certify ready even if local terminal proof is observed. | `degraded` by default; #50 wires tmux direct with pinned model args and upgrades only after matching direct smoke evidence. | `adapters/first-batch-harnesses.ts` Reasonix degraded reason path; `scripts/smoke/harnesses.ts`; `scripts/smoke/first-batch-direct.ts`. |
