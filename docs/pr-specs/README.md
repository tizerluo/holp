...
- [Issue #97 - Real Agent-Interactive HOLP Harness Workspace Entry](./issue-97-real-agent-interactive-harness-workspace-entry.md):#91/#94 后暴露的真实交互缺口修复 PR。它暂停 #96 visual-only 路线,先让 cmux 中的真实 Codex Controller Agent CLI 成为人机入口:用户直接和 Codex 对话,Controller 通过 HOLP broker/client 调 worker,Sidecar 旁路实时显示状态;Kimi controller path 在可验证前保持 degraded,不声明 `cmux-ready`、#41 sufficiency 或 #36 readiness。
- [Issue #99 - `holp` Native-Style Same-Pane CLI Entry](./issue-99-holp-native-same-pane-cli-entry.md):#97 后的入口收敛 PR。它把用户入口改成类似 `claude` / `codex` / `opencode` 的 `holp` 命令,默认在单个 cmux terminal pane 内用 HOLP-owned tmux split 同时呈现真实 Codex Controller CLI 与 Sidecar TUI;用户入口只跑真实 worker,不暴露 fake/demo path,不声明 `cmux-ready`、#41 sufficiency 或 #36 readiness。
- [Issue #103 - Harness Workspace Continuity Affordance Completion](./issue-103-harness-workspace-continuity-affordances.md):#99/#102 后的 continuity/operator 补修 PR。它补齐 attach command 派生、rerun goal 暴露,并把 continue unsupported 从 broad degraded reason 收窄为 action-level limitation;不改 daemon/core/protocol,不声明 `cmux-ready`、#41 sufficiency 或 #36 readiness。
