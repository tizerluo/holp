...
- [Issue #45 - Multi-Agent CLI Runtime Surface Completion](./issue-45-multi-agent-cli-runtime-surface-completion.md):#41 learned-model data sufficiency 的前置 runtime-surface parity + terminal-consumer smoke 阶段。PR14 是 first-batch harness pilot,不是 bounded CLI cohort 的 headless + ACP/native-or-bridge + direct session 完整候选空间证明,也不是 cmux-ready 证明。
