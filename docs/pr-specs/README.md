...
- [Issue #85 - Interactive HOLP Harness Workspace Entry](./issue-85-interactive-harness-workspace-entry.md):#76 human validation 后的 blocker 修复 PR。它补一个 opt-in cmux interactive launcher,让用户能在真实 Controller Agent CLI pane 中交互,同时看到 honest/degraded-aware HOLP Sidecar/Evidence/Replay;不把 gate 改为 `allowed`,不声明 `cmux-ready` 或 #41 sufficiency。
