...
- 当前 `adapters/` 包含 contract、demo/test fake backend、Codex app-server real adapter 和 native-claude headless reviewer partial;`createDefaultAdapterRegistry()` 把 `"mcp-codex"` 接到三个独立 surface(headless → app-server, acp → AcpBackend, direct_user_session → DirectTmuxBackend)、把 `"native-claude"` 接到 Claude Code `-p --output-format json`;`codexRuntimeSurfaces()` 对 acp 和 direct 默认 fail-closed(`codex_acp_smoke_not_enabled` / `codex_direct_smoke_not_enabled`),需 `HOLP_REAL_CODEX_SMOKE=1` 才能升 ready。#48 已落地。
