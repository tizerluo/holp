...
- 当前 `runEngine` 已有 PR9 reviewer executor hook:`fake` reviewer 与 `mcp-codex` read-only reviewer pilot 都必须通过 canonical parser/validator 和 read-only attestation 后才会成为 completed vote。
