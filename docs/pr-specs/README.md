...
- [Issue #95 - Harness Workspace TUI Sidecar Visual Shape](./issue-95-harness-workspace-tui-sidecar-visual-shape.md):#91/#94 后的 Go TUI 视觉形态补强。它只把现有 Sidecar 渲染推进到更接近 `focus-overview.png` / `focus-inspect.png` 的 Overview/Inspect 信息架构,不绘制 fake Controller pane、不改 broker/protocol/cmux topology、不移动 #76 human gate。
