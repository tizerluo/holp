...
export interface StopDecisionSampleV1 {
  readonly version: "StopDecisionSample.v1";
  readonly sample_id: string;
  readonly run_id: string;
  readonly step_index: number;
  readonly state: DispatchStateV1;
  readonly action: WorkPlanV1 & { readonly kind: "terminal" };
  readonly reward: 1 | -1 | null;
  readonly reward_basis: string;
  readonly reward_policy_version: typeof REWARD_POLICY_VERSION;
  readonly ts: number;
...
export function exportStopDecisionSamplesJsonl(decisions: readonly DecisionRecord[]): string {
  return exportStopDecisionSamples(decisions)
    .map((sample) => JSON.stringify(sample))
    .join("\n");
}
...
export function exportStopDecisionSamples(decisions: readonly DecisionRecord[]): readonly StopDecisionSampleV1[] {
  const terminalByRunId = new Map<string, DecisionRecord>();
  for (const decision of decisions) {
    if (decision.decision_type === "run_terminal" && decision.run_id) {
      terminalByRunId.set(decision.run_id, decision);
  ...
  return decisions
    .filter((decision) => decision.decision_type === "stop_decision_sample_recorded")
    .map((decision) => stopSampleFromDecision(decision, terminalByRunId.get(decision.run_id ?? "")))
    .filter((sample): sample is StopDecisionSampleV1 => sample !== null);
}
...
function stopSampleFromDecision(
  decision: DecisionRecord,
  terminal: DecisionRecord | undefined,
): StopDecisionSampleV1 | null {
  if (!isRecord(decision.data)) return null;
  const state = decision.data.state;
  const action = decision.data.action;
  if (!isDispatchState(state) || !isTerminalWorkPlan(action)) return null;

  const reward = attributeStopDecisionReward(terminal);
  return {
    version: "StopDecisionSample.v1",
    sample_id: typeof decision.data.sample_id === "string"
      ? decision.data.sample_id
      : `${state.run_id}:stop:${state.step_index}`,
    run_id: state.run_id,
    step_index: state.step_index,
    state,
    action,
...
function attributeStopDecisionReward(terminal: DecisionRecord | undefined): RewardAttribution {
  if (!terminal) return { reward: null, reward_basis: "terminal_missing" };
  ...
  if (state === "merged") return { reward: 1, reward_basis: "terminal_merged" };
  if (state === "blocked" || state === "gave_up") {
    return { reward: -1, reward_basis: "terminal_failed" };
  }
  return { reward: null, reward_basis: "not_attributable" };
}
...
function isTerminalWorkPlan(value: unknown): value is WorkPlanV1 & { readonly kind: "terminal" } {
  return isWorkPlan(value) && value.kind === "terminal";
}
