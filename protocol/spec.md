...
- `artifact_refs` 不可用 → server 不得发 artifact ref(大 payload 内联截断);凡 spec 中放 envelope 的位置(consensus `reviews[].findings`、approval `details`)改放**内联降级形态**(见 §6.1、§7)。
...
- **unknown agent**(从未在本连接 flock response 出现过的 id)→ error `invalid_request`(JSON-RPC 标准 -32600)。
- **known 但 status=rejected**(声明/发现过但不可调度)→ 该 agent 视为 panel/role 的无效成员:用于 reviewer panel 时按 §6.2 走 `invalid_quorum`(panel 含 rejected agent);用于单点 role(coder/architect 等)时按 §10.1 走 `unsupported_transport`/`missing_auth`。
...
`orchestrate.run` 在受理前做硬校验(见 §4.2、§6.2、§10):agent 引用合法性 + role 匹配、共识可满足性、approval 能力匹配、transport 可用。校验不过 → 立即 error response,不进 run。
...
**`categories` 语义**:可选的**订阅白名单**。给出 → 只推列出 category 的事件;**省略或 `null` → 订阅全部 category**(不是不推)。空数组 `[]` 非法 → `invalid_request`(订阅了个寂寞)。`categories` 是**封闭枚举**,合法值仅 §5 末列出的五个:`run` | `agent` | `consensus` | `approval` | `lifecycle`;含未知 category 字符串 → `invalid_request`。心跳不受 `categories` 过滤,由 `include_heartbeats` 单独控制。
...
> 大 payload(diff 全文等)默认走 **artifact envelope**(见 §8),不内联;client 不支持 `artifact_refs` 时按 §2/§6.1/§7 降级为内联截断。
...
**错误码定序**(三段,不重叠):
1. `invalid_request`(§4.2)管**引用合法性**:panel/role 含 unknown agent(本连接 flock 从未出现)。最先校验。
2. `invalid_quorum`(§10)管 quorum **形状**:quorum≤0、quorum>panel 规模、panel 空、panel 含 rejected agent、全部 excluded。
3. `quorum_unsatisfiable`(§10)管**策略性不足**:形状合法但排除作者后 eligible 不够。

**顺序固定:引用合法性(unknown)→ 形状(invalid_quorum)→ 策略(quorum_unsatisfiable)**。到第 2 段时 panel agent 已全部 known。
...
| -32007 | `invalid_quorum` | quorum 参数非法:≤0 / > panel / panel 空 / panel 含 rejected agent / 全部 excluded(unknown agent 不归这里,走 `invalid_request` §4.2) |
...
> 引用合法性:unknown agent(从未在本连接 flock 出现)走 `invalid_request`(-32600,§4.2/§10.1);known-but-rejected 按用途分流到 `invalid_quorum`(reviewer panel)或 `unsupported_transport`/`missing_auth`(单点 role)。
...
- `orchestrate.run` 受理时,**单点 role**(coder/architect/tester 等)所需 agent 是 rejected status → error `unsupported_transport` 或 `missing_auth`(此时才是 JSON-RPC error,因为 run 无法进行)。**reviewer panel** 含 rejected agent 不走这里,走 §6.2 的 `invalid_quorum`(形状校验,panel 含 rejected)。引用未声明的 unknown agent → `invalid_request`(§4.2)。
...
- **v0.1.4**:跨仓 review(对照 cmux/warp/loopwright/happier/spawn/skypilot 真实代码)后补互操作缺口。**P1**:`artifact_refs` 不可用时 consensus `findings` / approval `details` 的内联降级形态(§2/§6.1/§7);`orchestrate.run` 的 agent 引用绑定 flock + role 校验(§4.2,新错误码 `role_unsupported` -32018);`events.subscribe.categories` 语义(白名单/省略=全订)+ 五 category 封闭枚举(§5)。**P2**:§7 race 表述对 server-timeout 分支修正;§10.1 单点 role vs reviewer panel 的 rejected 分流交叉引用;seq 从 1 起的边界定义(§5);happier 权限枚举更正(五值,非三值)+ 补 loopwright `reviewerCandidates` 归因(positioning)。
