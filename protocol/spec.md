...
> **版本**:v0.1.x (draft,当前 v0.1.3)
> **状态**:草稿,参考实现进行中
> **变更历史**:v0.1 → v0.1.1(传输/共识/审批/版本/flock 首轮返工)→ v0.1.2(把 v0.1.1 的「表面改」逼到可执行)→ v0.1.3(第三轮 review 后修 5 个阻塞缺口:quorum 公式 / artifact 强制 content / approval cancel / task.cancel schema / capability required scope + 错误码定序 + 跨文件扫净)。第四轮 review 判「偏 yes,实现前无 P0」。变更明细见文末。
...
- `approval.kinds`:取交集;**任一方 `approval.required:true` 且 kinds 交集为空** → `initialize` 拒绝(required 无法满足,`capability_required_but_unsupported`);**双方都非 required 但 kinds 空交集** → 不拒绝连接,run 触发该 kind 时按 §7/§11 降级或 escalate。
...
> v0.1.3 修正(codex 第三轮 P1):v0.1.2 说 approval_cancelled 可由 client/server 主动取消,但控制面没 `approval.cancel` 方法。v0.1.3 **规定:client 不能单独取消 approval——approval 取消只由 `task.cancel`(取消整个 run,§7.5)导致**。没有 `approval.cancel` 方法。要中止 approval 的唯一途径是 `task.cancel` 该 run,server 随后对所有 pending approval 发 `approval_cancelled`。这避免「client 取消 approval 但 run 还在跑」的不一致。
