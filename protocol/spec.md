...
- 任一方 `required:true` 但对方 `supported:false` → 该能力不可用;若该能力是某 run 的硬前提 → `orchestrate.run` 受理前 reject(错误 `capability_required_but_unsupported`,见 §10);若是连接级硬要求 → `initialize` 拒绝(`capability_required_but_unsupported`)。
...
- `approval.kinds`:取交集;run 触发的 kind 不在交集 → 按 §7/§11 降级或 escalate。
...
**第 1 段:静态上界校验(`orchestrate.run` 受理时,无作者信息)**
- 校验 `roles.reviewer.panel` 的规模上界:`panel 中 enabled 且 capability 可用的 agent 数 ≥ quorum + 1`(留出作者占一席的余量,因作者待定)。
- 不满足 → error `quorum_unsatisfiable`(见 §10)或按 `policy.on_quorum_unsatisfiable`(此时只能 `reject`,因为还没法 `ask_human` 具体内容)。
- 这一段只挡「panel 怎么排都不够」的死配置。
...
两段式的好处:受理时挡死配置(早失败),consensus 前挡动态不足(作者已定时的精确判断)。
...
终态事件 name:`approval_resolved`(人决定)| `approval_expired`(超时)| `approval_cancelled`(server/client 主动取消)。三条事件的 payload 用**统一的 `state` 字段**区分:
...
{ "method": "events.event", "params": { "seq": 41, "category":"approval",
  ...
  "payload": { "approval_id":"ap_1", "state":"resolved", "decision":"approved", "by":"user:tizer" } } }
...
```
**Response**(小 artifact 内联 content,大 artifact 给 content_ref + token):
```jsonc
{ "jsonrpc": "2.0", "id": 7, "result": {
  "artifact_id": "art_diff_1", "envelope": { /* §8.1 全字段 */ },
  "content": "diff --git a/...",          // 内联(小)
  "truncated": false
} }
```
或大 artifact:
```jsonc
{ "result": { "artifact_id":"art_diff_1", "envelope":{...},
  "content_ref": "holp-art://art_diff_1?token=xxx",  // 带 token 的临时读取路径
  "truncated": false } }
```
