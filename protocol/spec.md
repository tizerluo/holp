...
跨厂商共识评审是 HOLP 写进协议的真差异(Oz 多 harness 并行但无共识聚合;happier reviews 是单引擎)。
...
- ❌ 未做(不声称):native-claude / mcp-codex 真接线(待 adapter 真实现 / 接 happier 方言库);acp;Web 传输;Remote(wire 不含)。
